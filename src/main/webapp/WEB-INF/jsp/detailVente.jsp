...
<body>
	...
	<main>
		...
		%>
		<%
		if (!encheres.isEmpty() && av.getArticleStatus() == ArticleStatus.ET
				&& (Integer) session.getAttribute("noUtilisateur") == (int) u.getNoUtilisateur()) {
		%>
		<h2 style="color: black">Vous avez remporté cette enchère,
			félicitations !</h2>
		<%}%>
		<div class="vente">
			...
			<div class="detail-vente">
				...
				<h5>
					(<%=c != null ? c.getLibelle() : "La catégorie n'existe plus"%>)
				</h5>
				<div class="name">
					<span>Vendu par</span> <a
						href="/ENI-enchere/InfosProfil/<%=av.getNoUtilisateur()%>"> <%=av.getNomPrenomAuteur()%></a>
				</div>
				<p>
					Description :
					<%=av.getDescription()%></p>
		...
		if (av.getArticleStatus() == ArticleStatus.EC
				&& (int) av.getNoUtilisateur() != (Integer) session.getAttribute("noUtilisateur")
				&& session.getAttribute("desactive") == null) {
		%>
		<form action="/ENI-enchere/DetailVente/<%=av.getNoArticle()%>"
			method="POST">
			<div class="vente-container">
				<div class="detail-vente-field">
					<label for="proposition">Ma proposition</label> <input
						id="proposition" name="proposition" type="number"
						value="<%=av.getPrixVente() + 1%>">
				</div>
